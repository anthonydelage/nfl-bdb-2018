DROP VIEW IF EXISTS `ad-big-data-bowl.workspace.passes_enhanced`;
CREATE VIEW `ad-big-data-bowl.workspace.passes_enhanced` AS
WITH
w_tracking AS (
  SELECT
    *,
  ...
  (
    SELECT
      *,
      ...
      MIN(pass_ball_distance) OVER (PARTITION BY gameplay_id, player_side) AS min_pass_ball_distance,
      MIN(arrived_ball_distance) OVER (PARTITION BY gameplay_id, player_side) AS min_arrived_ball_distance,
      MIN(arrived_pred_ball_distance) OVER (PARTITION BY gameplay_id, player_side) AS min_arrived_pred_ball_distance
    FROM
      (
        SELECT
          *,
          (snap_x - snap_ball_x) * play_direction_multiplier AS snap_x_rel,
          (pass_x - snap_ball_x) * play_direction_multiplier AS pass_x_rel,
          (arrived_x - snap_ball_x) * play_direction_multiplier AS arrived_x_rel,
          snap_velocity_x * play_direction_multiplier AS snap_velocity_x_rel,
          pass_velocity_x * play_direction_multiplier AS pass_velocity_x_rel,
          arrived_velocity_x * play_direction_multiplier AS arrived_velocity_x_rel,
          snap_accel_x * play_direction_multiplier AS snap_accel_x_rel,
          pass_accel_x * play_direction_multiplier AS pass_accel_x_rel,
          arrived_accel_x * play_direction_multiplier AS arrived_accel_x_rel,
          (pass_ball_x - snap_ball_x) * play_direction_multiplier AS pass_ball_x_rel,
          (arrived_ball_x - snap_ball_x) * play_direction_multiplier AS arrived_ball_x_rel,
          (CASE WHEN play_direction_multiplier > 0 THEN snap_direction
            WHEN snap_direction > 180 THEN snap_direction - 180
            WHEN snap_direction < 180 THEN snap_direction + 180
            END) AS snap_direction_rel,
          (CASE WHEN play_direction_multiplier > 0 THEN pass_direction
            WHEN pass_direction > 180 THEN pass_direction - 180
            WHEN pass_direction < 180 THEN pass_direction + 180
            END) AS pass_direction_rel,
          (CASE WHEN play_direction_multiplier > 0 THEN arrived_direction
            WHEN arrived_direction > 180 THEN arrived_direction - 180
            WHEN arrived_direction < 180 THEN arrived_direction + 180
            END) AS arrived_direction_rel
        FROM
          (
            SELECT
              *,
              MAX(qb_direction_multiplier) OVER (PARTITION BY gameplay_id) AS play_direction_multiplier,
            ...
            FROM
              (
                SELECT
                  *,
                  (CASE WHEN position = 'QB' AND snap_direction > 180 THEN 1
                    WHEN position = 'QB' AND snap_direction <= 180 THEN -1
                    END) AS qb_direction_multiplier,
                ...
                FROM
                  (
                    SELECT
                      CONCAT(CAST(t_play.game_id AS STRING), '-', CAST(t_play.play_id AS STRING)) AS gameplay_id,
                      t_play.*,
